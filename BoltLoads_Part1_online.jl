...
Hover over the square bolt pattern below to probe the x & y coordinates.
...
md"We can manually enter the bolt pattern points as an x & y vector to enable further analysis; we will start at the point in the top left and enter all coordinates for the x & y vectors in the clockwise direction:"
...
md"#### Exercise 🤔
Examine the bolt pattern below by hovering over each bolt to probe the x & y coordinates.
"
...
# ╔═╡ 18d84286-9f13-40ff-aee0-e88ce1499746
md"**[Ex 1.1] Manually enter the values of `x2`, `y2` and `p2`for the five bolts in the bolt pattern shown above:**

Enter the values for each coordinate starting from the point on the left and moving to the point on the right.

Note: press `shift` + `enter` to evaluate each cell."
...
p8 = circle(r=360mm/2, N=3, θ_start = 45)
...
p9 = circle(r=360mm/2, N=3, θ_start=90)
...
* To type $\theta\_start$ in julia type: `\theta`, then press `tab` and then type `_start`.
* If you do not have `julia` installed on your computer, you may not be able to have theta displayed as a symbol; to complete this exercise please copy and paste $\theta\_start$ from one of the cells above as applicable.
"
...
begin
	...
	function exp11()
		if p11 == circle(r=6.25inch/2, N=5, θ_start=30)
			correct(md"""**Great!**  You figured it out.  Keep going.
	...
	function exp12()
		if p12 == circle(r=11.7cm, N=6, θ_start=90-37)
